...
import { 
  ...
  Spline, BarChartBig, Palette, LayoutTemplate as TemplateIcon, Edit3, // Renamed LayoutTemplate to avoid conflict
  ImageIcon, LayoutDashboard, SlidersHorizontal, UserPlus, PlusCircle, MailPlus, CalendarDays, Smile
} from 'lucide-react'; 
import {
  Sidebar,
  SidebarContent,
...
export function MainLayout({ children, pageTitle }: { children: ReactNode; pageTitle: string }) {
  ...
  const initialNavItems: NavItemConfig[] = React.useMemo(() => [
    { href: '/', label: 'Dashboard', icon: <Home className="h-5 w-5" /> },
    {
      ...
      subItems: [
        ...
        { href: '/social-media/scheduled-posts', label: 'Scheduled Posts', icon: <CalendarDays className="h-4 w-4" /> },
        { href: '/social-media/asset-library', label: 'Asset Library', icon: <ImageIcon className="h-4 w-4" /> },
      ...
      if (item.isGroup && item.subItems) {
        ...
        item.subItems.forEach(subItem => {
          if (subItem.isSubGroupHeader && subItem.subSubItems) {
            const isSecondLevelActive = subItem.subSubItems.some(subSub => subSub.href && pathname.startsWith(subSub.href) && (subSub.href === '/' ? pathname === '/' : true));
            if (isSecondLevelActive) {
              isTopLevelActive = true;
              newOpenSecondLevelGroups[`${item.label}-${subItem.label}`] = true;
            }
          } else if (subItem.href && pathname.startsWith(subItem.href) && (subItem.href === '/' ? pathname === '/' : true)) {
            isTopLevelActive = true;
          }
        });
        if (isTopLevelActive) {
        ...
        <SidebarContent className="p-2">
          <SidebarMenu>
            {initialNavItems.map((item) => {
              const isTopLevelLinkActive = item.href && pathname.startsWith(item.href!) && (item.href === '/' ? pathname === '/' : true);
              ...
              if (item.isGroup && item.subItems) {
                ...
                const isGroupActive = item.subItems.some(sub => {
                  if (sub.isSubGroupHeader && sub.subSubItems) {
                    return sub.subSubItems.some(subSub => subSub.href && pathname.startsWith(subSub.href) && (subSub.href === '/' ? pathname === '/' : true));
                  }
                  return sub.href && pathname.startsWith(sub.href) && (sub.href === '/' ? pathname === '/' : true);
                });
                      ...
                      <TooltipTrigger asChild>
                        <SidebarMenuButton
                          className={cn(
                            ...
                            (isGroupActive && !isCurrentItemGroupOpen) && "bg-sidebar-primary text-sidebar-primary-foreground hover:bg-sidebar-primary/90",
                            isCurrentItemGroupOpen && isGroupActive && "bg-sidebar-accent text-sidebar-accent-foreground",
                            isCurrentItemGroupOpen && !isGroupActive && "bg-sidebar-accent/50 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
                            !isCurrentItemGroupOpen && !isGroupActive && "hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
                          )}
                          onClick={() => toggleTopLevelGroup(item.label)}
                          aria-expanded={isCurrentItemGroupOpen}
                          aria-current={(isGroupActive && !isCurrentItemGroupOpen) ? "page" : undefined}
                        >
                      ...
                      <SidebarMenuSub>
                        {item.subItems.map((subItem) => {
                          if (subItem.isSubGroupHeader && subItem.subSubItems) {
                            ...
                            const isSecondLevelGroupActive = subItem.subSubItems.some(subSub => subSub.href && pathname.startsWith(subSub.href) && (subSub.href === '/' ? pathname === '/' : true));
                                  ...
                                  <TooltipTrigger asChild>
                                    <SidebarMenuButton
                                      ...
                                      className={cn(
                                        ...
                                        (isSecondLevelGroupActive && !isCurrentSecondLevelGroupOpen) && "bg-sidebar-primary text-sidebar-primary-foreground hover:bg-sidebar-primary/90",
                                        isCurrentSecondLevelGroupOpen && isSecondLevelGroupActive && "bg-sidebar-accent text-sidebar-accent-foreground",
                                        isCurrentSecondLevelGroupOpen && !isSecondLevelGroupActive && "bg-sidebar-accent/50 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
                                        !isCurrentSecondLevelGroupOpen && !isSecondLevelGroupActive && "hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
                                      )}
                                ...
                                {isCurrentSecondLevelGroupOpen && (
                                  <SidebarMenuSub className="!ml-2 !pl-2 !border-l-2 !border-sidebar-border/50"> 
                                    {subItem.subSubItems.map((subSubItem) => {
                                      const isSubSubItemActive = subSubItem.href && pathname.startsWith(subSubItem.href) && (subSubItem.href === '/' ? pathname === '/' : true);
                                      return (
                                        <SidebarMenuSubItem key={subSubItem.href || subSubItem.label}>
                                          <Tooltip>
                                            <TooltipTrigger asChild>
                                              <Link href={subSubItem.href!} passHref legacyBehavior={true}>
                                                <SidebarMenuSubButton
                                                  size="sm"
                          ...
                          // Direct sub-item link (not a sub-group header)
                          const isSubItemActive = subItem.href && pathname.startsWith(subItem.href) && (subItem.href === '/' ? pathname === '/' : true);
                          return (
                            <SidebarMenuSubItem key={subItem.href || subItem.label}>
                               <Tooltip>
                                <TooltipTrigger asChild>
                                  <Link href={subItem.href!} passHref legacyBehavior={true}>
                                    <SidebarMenuSubButton
